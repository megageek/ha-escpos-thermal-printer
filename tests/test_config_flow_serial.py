...
class TestConnectionTypeStep:
    """User step routes to serial when Serial is picked."""
...
class TestSerialStep:
    """Tests for the serial configuration step."""
    ...
    async def test_empty_port_returns_error(self, hass):
        ...
        result = await flow.async_step_serial(
            {CONF_SERIAL_PORT: "", CONF_BAUDRATE: 9600, "timeout": 4.0, "profile": ""}
        )
        assert result["type"] == "form"
        ...
        result = await flow.async_step_serial(
            {
                ...
                CONF_BAUDRATE: 1234,  # not in _BAUDRATE_CHOICES
                "timeout": 4.0,
                "profile": "",
            }
        )
